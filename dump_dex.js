...
Interceptor.attach(defineClass, {
    onEnter: function (args) {
        ...
        if(classname == "Lcom/qihoo/sdk/report/d/a;") { // 修改相关类
                console.log("[+] class name: " + classname);
                ...
                //保存在应用目录下
                var file = new File("/data/data/com.qihoo.srouter.n300/files/dumpdex.dex","wb");
                
                for (; i < count ; i++) {
                    if ((lastbuf_len > 0) && (i == (count - 1))) {
                        console.log("[*] mem off: " + i*mapx_buf_len);
                        console.log("[*] mem last off: " + lastbuf_len);
                        file.write(Memory.readByteArray(ptr(dp + i*mapx_buf_len) , lastbuf_len));
                        ...
                        file.write(Memory.readByteArray(ptr(dp + i*mapx_buf_len) , mapx_buf_len))
                    } 
                }
                file.close()
                console.log("[+] finish to dump dex .../data/data/com.qihoo.srouter.n300/files/dumpdex.dex");
            }
        }
    });
    });
